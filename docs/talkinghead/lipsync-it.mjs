...
class LipsyncIt {
  ...
  constructor() {
    ...
    const ops = {
      '#': '[AEIOUÀÈÉÌÒÙ]+', // One or more vowels including accented ones
      ...
      '+': '[EIÉÈÌ]', // One of E, I including accented versions
    ...
    Object.keys(this.rules).forEach( key => {
      this.rules[key] = this.rules[key].map( rule => {
        ...
        o.regex = new RegExp(exp, 'i'); // Add case-insensitive flag
  ...
  preProcessText(s) {
    return s
      .replace(/[#_*\":;]/g,'') // Remove unwanted characters but NOT accented ones
      ...
      // CRITICAL: DO NOT normalize accented characters - preserve à, è, é, ì, ò, ù, etc.
      // This is the key fix for the "Née" issue
      .trim();
  }
